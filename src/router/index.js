...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.meta.requiresAdmin && authStore.user?.role !== 'admin') {
    ...
    window.showToast('error', 'Access Denied', 'You need admin privileges to access this page')
    next({ name: 'Home' })
    return
  }
  ...
  if (to.meta.requiresVolunteer && !['admin', 'volunteer'].includes(authStore.user?.role)) {
    ...
    window.showToast('error', 'Access Denied', 'You need volunteer privileges to access this page')
    next({ name: 'Home' })
    return
  }
  ...
  if (to.meta.requiresBeneficiary && authStore.user?.role !== 'beneficiary') {
    ...
    window.showToast('error', 'Access Denied', 'You need beneficiary privileges to access this page')
    next({ name: 'Home' })
    return
  }
